...
/** GraphAlgorithms.cpp (2) */
PathResult GraphAlgorithms::dfsPath(
    const Graph& graph,
    const std::string& startName, const std::string& targetName)
{
    ...
    std::function<bool(int)> dfs = [&](int u)  // lambda function dfs()
    {
        visited[u] = true;
...
/** GraphAlgorithms.cpp (3) */
PathResult GraphAlgorithms::bfsShortestPath(
    const Graph& graph,
    const std::string& startName, const std::string& targetName)
{
    int start  = graph.idOf(startName);
...
/** GraphAlgorithms.cpp (5) */
PathResult GraphAlgorithms::dijkstraShortestPath(
    const Graph& graph,
    const std::string& startName, const std::string& targetName)
{
    int start  = graph.idOf(startName);
...
/* ============================================================
 *  DisjointSet (Kruskal 내부 사용)
 * ============================================================ */
class DisjointSet {
private:
    std::vector<int> parent_;   // parent of vertex
    std::vector<int> rank_;     // sub-MST 의 높이
public:
                                    ...
                                    const std::string& startName)
{
    ...
    while (!pq.empty() &&
        ...
        if (selected[cur.edge.to]) continue;    // 이미 선택 → skip
